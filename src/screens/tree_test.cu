...
void TreeTest::render(float dt) {
  ...
  auto &rect = game.get_resources().rect_renderer;
  auto &line = game.get_resources().line_renderer;
  bold.set_transform(vp.get_transform());
  rect.set_transform(vp.get_transform());
  line.set_transform(vp.get_transform());

  bool update_render =
      mixing || mutating_len_rot || updating_parallel || mutating_pos || updating_cpu;
  ...
  //         read_tree_device.copy_to_host(read_tree);
  //         auto start = std::chrono::steady_clock::now();
  //         trees::mutate_len_rot(read_tree, game.get_resources().generator, 0.0f, 0.01f);
  //         auto end = std::chrono::steady_clock::now();
  //         auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
  ...
  //         read_tree_device.copy_to_host(read_tree);
  //         auto start = std::chrono::steady_clock::now();
  //         trees::mutate_pos(read_tree, game.get_resources().generator, 1.0f);
  //         auto end = std::chrono::steady_clock::now();
  //         auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(end - start);
  ...
  //
  //         trees::render_tree_kernel<<<grid, block>>>(static_cast<unsigned int *>(vbo_cuda_map),
  //         read_tree_device_ptrs, node_count);
  //         // wait for kernel to finish
  //         cudaDeviceSynchronize();
  //         line.cudaUnmapBuffer();
  //     } else {
  //         trees::render_tree(line, read_tree, game.get_resources().generator,
  //         vp.get_transform());
  //     }
  //
  // }
  ...
  bold.add_text(hud_vp.get_left() + padding, hud_vp.get_bottom() + padding + 120, 300, mix_time,
                glm::vec4(0.75), FontRenderer::HAlign::RIGHT);
  bold.add_text(hud_vp.get_left() + padding, hud_vp.get_bottom() + padding + 60, 300, mutate_time,
                glm::vec4(0.75), FontRenderer::HAlign::RIGHT);
  bold.add_text(hud_vp.get_left() + padding, hud_vp.get_bottom() + padding, 300, update_time,
                glm::vec4(0.75), FontRenderer::HAlign::RIGHT);
...
bool TreeTest::handle_input(SDL_Event event) {
