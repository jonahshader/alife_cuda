...
void TexCUDATest::render(float dt) {
  ...
  // cudaTextureObject_t tex_obj = rect.create_texture_object();
  // if (tex_obj == 0)
  // {
  ...
  if (cudaStatus != cudaSuccess) {
    ...
    // rect.destroy_texture_object(tex_obj);
    rect.cuda_unmap_texture();
    cudaFree(d_output);
    return;
  }
...
bool TexCUDATest::handle_input(SDL_Event event) {
