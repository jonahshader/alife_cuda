...
#include "profiler_gui.cuh"

#include <algorithm>
#include <numeric>
#include <vector>

ProfilerGui::ProfilerGui() : time_range(5.0f), window_flags(ImGuiWindowFlags_None) {}

void ProfilerGui::render() {
  if (!ImGui::Begin("Timing Profiler", nullptr, window_flags)) {
    ImGui::End();
    return;
  }

  auto &profiler = TimingProfiler::get_instance();
  const auto names = profiler.get_profile_point_names();
  if (names.empty()) {
    ImGui::End();
    return;
  }
  ...
  for (const auto &name : names) {
    if (auto point = profiler.get_point(name)) {
      total_avg += point->get_average_duration();
    }
  }
  ...
  for (const auto &name : names) {
    if (auto point = profiler.get_point(name)) {
      max_time = std::max(max_time, (float)point->get_average_duration());
    }
  }
  ...
  for (const auto &name : names) {
    ...
    // Checkbox and label
    bool enabled = point->is_enabled();
    if (ImGui::Checkbox(("##" + name).c_str(), &enabled)) {
      if (enabled)
        profiler.enable_point(name);
      else
        profiler.disable_point(name);
    }
    ImGui::SameLine();

    // Get the average time and format the label
    float avg_time = point->get_average_duration();
    ...
    ImU32 color = get_color_for_duration(avg_time);
...
ImU32 ProfilerGui::get_color_for_duration(float duration) const {
  float t;
