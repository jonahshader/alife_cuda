...
std::vector<BranchNodeFull> build_tree(uint32_t num_nodes, std::default_random_engine &rand,
  ...
  for (int i = 1; i < num_nodes; i++) {
    ...
    nodes.emplace_back(BranchNodeFull{
        ...
        .nav = BranchNav{.id = static_cast<uint32_t>(i)}});
  }
...
std::vector<trees2::BranchNode> strip_nav(const std::vector<BranchNodeFull> &nodes) {
  ...
  for (const auto &node : nodes) {
    ...
    stripped.push_back(trees2::BranchNode{
        .core = node.core,
        .stats = node.stats,
        .ch = trees2::BranchShape{.start = children_start, .count = num_children}});
  }
  return stripped;
}

std::vector<trees2::BranchNode>
build_tree_optimized(uint32_t num_nodes, std::default_random_engine &rand, glm::vec2 start_pos) {
  return strip_nav(sort_tree(build_tree(num_nodes, rand, start_pos)));
}

TreeBatch concatenate_trees(const std::vector<Tree> &trees,
                            const std::vector<trees2::TreeData> &tree_data) {
  TreeBatch batch{};
...
trees2::TreeBatch make_batch(uint32_t node_count, uint32_t tree_count,
  ...
  for (int i = 0; i < tree_count; ++i) {
    ...
    //        trees.push_back(build_tree_optimized(NUM_NODES, rand, glm::vec2(spawn_dist(rand),
    //        spawn_dist(rand))));
...
// render_tree, render_tree_kernel, and render_trees_cuda are in graphics/tree_render.cu

__global__ void update_rot_kernel(const trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                                  size_t node_count) {
...
#pragma omp parallel for
  for (int i = 0; i < read_batch.trees.size(); ++i) {
    ...
    const float new_angle =
        std::atan2(avg_end_pos.y - avg_start_pos.y, avg_end_pos.x - avg_start_pos.x);
    ...
    if (has_parent) {
      ...
      auto new_start = avg_center - glm::vec2(std::cos(new_angle), std::sin(new_angle)) *
                                        read.core.length / 2.0f;
...
#pragma omp parallel for
  for (int i = 0; i < read_batch.trees.core.abs_rot.size(); ++i) {
    ...
    if (has_parent) {
      ...
      for (auto j = read_ch.start[parent_index];
           j < read_ch.start[parent_index] + read_ch.count[parent_index]; ++j) {
        if (j != i) {
...
__host__ __device__ float smallest_angle_between(float angle1, float angle2) {
...
__global__ void fix_pos_kernel(const trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                               size_t node_count, float dt_inv, float vel_interp) {
  auto i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= node_count) {
    return;
  }
  ...
  if (has_parent) {
    new_start_pos = read_pos[parent_index] +
                    get_length_vec(read_abs_rot[parent_index], read_length[parent_index]);
  }
  ...
  // compute new angle
  const float new_angle =
      std::atan2(new_end_pos.y - new_start_pos.y, new_end_pos.x - new_start_pos.x);
...
__device__ float calc_torque(const float &min_thickness, const float &rot_delta) {
  // return rot_delta * min_thickness * trees2::TORQUE_PER_RAD;
  // float constrained_rot_delta = max(-static_cast<float>(M_PI) / 2, min(static_cast<float>(M_PI) /
  // 2, rot_delta));
  return rot_delta * pow(min_thickness, 4) * trees2::TORQUE_PER_RAD;
}

__global__ void integrate_kernel_condensed(const trees2::TreeBatchPtrs read,
                                           trees2::TreeBatchPtrs write, size_t node_count,
                                           float dt) {
  ...
  const auto mass = current_length * current_thickness * current_thickness *
                    static_cast<float>(M_PI) * trees2::MASS_PER_CUBIC_CM;
  ...
  const auto new_rot_vel =
      read.trees.core.rot_vel[i] * ANGULAR_DAMPENING + (is_impossible ? 0.0f : rot_acc * dt);
...
compute_tree_total_energies(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write, unsigned i) {
...
__global__ void compute_tree_total_energies_kernel(trees2::TreeBatchPtrs read,
                                                   trees2::TreeBatchPtrs write, size_t tree_count) {
...
void compute_tree_total_energies_cpu(trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                                     size_t tree_count) {
#pragma omp parallel for
...
__host__ __device__ inline void set_tree_total_energies(trees2::TreeBatchPtrs &read,
                                                        trees2::TreeBatchPtrs &write, unsigned i) {
...
__global__ void set_tree_total_energies_kernel(trees2::TreeBatchPtrs read,
                                               trees2::TreeBatchPtrs write, size_t tree_count) {
  const auto i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= tree_count) {
    return;
...
void set_tree_total_energies_cpu(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write,
                                 size_t tree_count) {
#pragma omp parallel for
  for (auto i = 0; i < tree_count; ++i) {
    set_tree_total_energies(read, write, i);
...
__host__ __device__ inline void mix_node_contents(trees2::TreeBatchPtrs &read,
                                                  trees2::TreeBatchPtrs &write, unsigned i) {
...
__global__ void mix_node_contents_kernel(trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                                         size_t tree_count) {
  const auto i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= tree_count) {
    return;
...
void mix_node_contents_cpu(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write,
                           size_t tree_count) {
#pragma omp parallel for
  for (auto i = 0; i < tree_count; ++i) {
    mix_node_contents(read, write, i);
...
__host__ __device__ inline void mix_node_give_take(trees2::TreeBatchPtrs &read,
                                                   trees2::TreeBatchPtrs &write, float dt,
                                                   unsigned i) {
...
__global__ void mix_node_give_take_kernel(trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                                          size_t tree_count, float dt) {
  const auto i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= tree_count) {
    return;
  }
...
void mix_node_give_take_cpu(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write,
                            size_t tree_count, float dt) {
#pragma omp parallel for
  for (auto i = 0; i < tree_count; ++i) {
    mix_node_give_take(read, write, dt, i);
...
void mix_node_contents_device_full(trees2::TreeBatchDevice &read_device,
  ...
  // std::cout << "last tree shape count: " << read_device.tree_shapes.count[tree_count - 1] <<
  // std::endl; std::cout << "last tree shape start: " << read_device.tree_shapes.start[tree_count -
  // 1] << std::endl;
  ...
  cudaDeviceSynchronize();
  // compute_tree_total_energies_kernel writes to tree_data.total_energy, so we need to swap the
  // pointers
...
void mix_node_contents_host_full(trees2::TreeBatch &read, trees2::TreeBatch &write) {
  ...
  // compute_tree_total_energies_cpu writes to tree_data.total_energy, so we need to swap the
  // pointers
...
__host__ __device__ inline void grow(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write,
                                     float dt, unsigned i) {
  const auto current_thickness = read.trees.stats.thickness[i];
  const auto current_length = read.trees.core.length[i];
  ...
  if (current_thickness < target_thickness || current_length < target_length) {
    ...
    // const auto new_mass = current_length * pow(sqrt(current_length / target_length) *
    // target_thickness, 2) * static_cast<float>(M_PI) * trees2::MASS_PER_CUBIC_CM + growth_energy *
    // trees2::MASS_PER_ENERGY; const auto new_length = pow((new_mass - growth_energy *
    // trees2::MASS_PER_ENERGY) * sqrt(target_length) / (static_cast<float>(M_PI) *
    // trees2::MASS_PER_CUBIC_CM), 2.0f/3.0f);
    const auto new_length =
        sqrt((new_mass)*target_length / (static_cast<float>(M_PI) * trees2::MASS_PER_CUBIC_CM *
                                         target_thickness * target_thickness));
...
__global__ void grow_kernel(trees2::TreeBatchPtrs read, trees2::TreeBatchPtrs write,
                            size_t node_count, float dt) {
  const auto i = blockIdx.x * blockDim.x + threadIdx.x;
  if (i >= node_count) {
    return;
  }
...
void grow_cpu(trees2::TreeBatchPtrs &read, trees2::TreeBatchPtrs &write, size_t node_count,
              float dt) {
#pragma omp parallel for
...
void init_trees(TreesState &state, uint32_t num_trees, uint32_t num_nodes,
  ...
  state.read_host = make_batch(num_nodes, num_trees, rand);
...
void update_trees(TreesState &state, float dt) {
  update_tree_cuda(state.read_device, state.write_device);
}

// render_trees_cuda is in graphics/tree_render.cu
...
void update_tree_cuda(trees2::TreeBatchDevice &read_batch_device,
  ...
  // we just wrote to write_batch_device's abs_rot, so we need to swap the pointers and re-acquire
  // ptrs
  write_batch_device.trees.core.abs_rot.swap(read_batch_device.trees.core.abs_rot);
  read_batch_ptrs.get_ptrs(read_batch_device);
  write_batch_ptrs.get_ptrs(write_batch_device);

  integrate_kernel_condensed<<<grid, block>>>(read_batch_ptrs, write_batch_ptrs, node_count,
                                              1 / 60.0f);
  cudaDeviceSynchronize();
  ...
  // we just wrote to write_batch_device's pos, abs_rot, current_rel_rot, vel, and rot_vel, so we
  // need to swap the pointers
  write_batch_device.trees.core.pos.swap(read_batch_device.trees.core.pos);
  // write_batch_device.trees.core.abs_rot.swap(read_batch_device.trees.core.abs_rot);
  // write_batch_device.trees.core.current_rel_rot.swap(read_batch_device.trees.core.current_rel_rot);
  write_batch_device.trees.core.vel.swap(read_batch_device.trees.core.vel);
  ...
  // we just wrote to write_batch_device's energy, thickness, and length, so we need to swap the
  // pointers
