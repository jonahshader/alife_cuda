...
__global__ void populate_grid_indices(float *x_particle, float *y_particle, int *grid_indices,
                                      int max_particles, int *particles_per_cell,
                                      int max_particles_per_cell, int grid_width, int grid_height) {
...
__global__ void compute_density(float *x_particle, float *y_particle, float *density,
                                int *grid_indices, int *particles_per_cell,
                                int max_particles_per_cell, int max_particles, int grid_width,
                                int grid_height, float kernel_vol_inv,
...
__global__ void compute_forces(float *x_particle, float *y_particle, float *x_velocity,
                               float *y_velocity, float *x_acceleration, float *y_acceleration,
                               int *grid_indices, int *particles_per_cell,
                               int max_particles_per_cell, float *density, int max_particles,
                               int grid_width, int grid_height, float smoothkernel_vol_inv,
                               float sharpkernel_vol_inv, particles::TunableParams params) {
  int i = blockIdx.x * blockDim.x + threadIdx.x; // particle id
  if (i >= max_particles) {
    return;
  }
  ...
  for (int yc = y_cell_min; yc <= y_cell_max; ++yc) {
    for (int xc = x_cell_min; xc <= x_cell_max; ++xc) {
      ...
      for (int slot_index = 0; slot_index < particles_in_cell; ++slot_index) {
        ...
        pressure_grad_x +=
            params.particle_mass * shared_pressure * kernel_derivative * dir_x / density_j;
        pressure_grad_y +=
            params.particle_mass * shared_pressure * kernel_derivative * dir_y / density_j;
  ...
  float acc_x =
      -pressure_grad_x / density_i + params.viscosity_multiplier * viscosity_force_x / density_i;
  float acc_y = -pressure_grad_y / density_i +
                params.viscosity_multiplier * viscosity_force_y / density_i +
...
__global__ void update_positions_velocities(float *x_particle, float *y_particle, float *x_velocity,
                                            float *y_velocity, float *x_acceleration,
                                            float *y_acceleration, int max_particles, float dt,
                                            float bounds_x, float bounds_y,
                                            particles::TunableParams params) {
  int i = blockIdx.x * blockDim.x + threadIdx.x; // particle id
  if (i >= max_particles) {
    return;
...
__global__ void render_kernel(unsigned int *circle_vbo, float *x, float *y, float radius,
                              size_t count) {
...
ParticleFluid::ParticleFluid(int width, int height, bool use_graphics) {
  ...
  std::uniform_real_distribution<float> dist_x(0.0f,
                                               width * 8 * 0.75f); // TODO: 8 is the soil cell size
  ...
  // there are some vectors that are present in the C++ host version, but missing in the CUDA
  // version.
...
void ParticleFluid::update(float dt) {
  ...
  reset_particles_per_cell<<<grid_dim, block>>>(grid_device.particles_per_cell.data().get(),
                                                grid_size);
  ...
  populate_grid_indices<<<grid_dim, block>>>(particles_device.x_particle.data().get(),
                                             particles_device.y_particle.data().get(),
                                             grid_device.grid_indices.data().get(), particle_count,
                                             grid_device.particles_per_cell.data().get(),
  ...
  block = dim3(256);
  grid_dim = dim3((particle_count + block.x - 1) / block.x);
  compute_density<<<grid_dim, block>>>(
      particles_device.x_particle.data().get(), particles_device.y_particle.data().get(),
  ...
  block = dim3(256);
  grid_dim = dim3((particle_count + block.x - 1) / block.x);
  update_positions_velocities<<<grid_dim, block>>>(
      particles_device.x_particle.data().get(), particles_device.y_particle.data().get(),
      particles_device.x_velocity.data().get(), particles_device.y_velocity.data().get(),
      particles_device.x_accel.data().get(), particles_device.y_accel.data().get(), particle_count,
      dt, bounds_x, bounds_y, params);
}
...
void ParticleFluid::render(const glm::mat4 &transform) {
  ...
  render_kernel<<<grid_dim, block>>>(
      static_cast<unsigned int *>(vbo_ptr), particles_device.x_particle.data().get(),
