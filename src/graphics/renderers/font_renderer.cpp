...
void FontRenderer::set_transform(glm::mat4 transform) {
  shader.use();
  shader.set_matrix4("transform", transform);
}
...
void FontRenderer::add_text(float x, float y, float target_width, const std::string &text,
                            glm::vec4 color, HAlign h_align) {
  float l = get_length(1, text);
  float size = target_width / l;
  switch (h_align) {
    case HAlign::LEFT:
      add_text(x - target_width, y, size, text, color);
      break;
    case HAlign::CENTER:
      add_text(x - target_width / 2, y, size, text, color);
      break;
    case HAlign::RIGHT:
