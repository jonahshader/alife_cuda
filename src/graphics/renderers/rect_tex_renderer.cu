...
cudaTextureObject_t RectTexRenderer::create_texture_object() {
  ...
  cudaError_t err = cudaCreateTextureObject(&tex_obj, &resDesc, &texDesc, nullptr);
  if (err != cudaSuccess) {
...
void RectTexRenderer::destroy_texture_object(cudaTextureObject_t tex_obj) {
