...
FontRenderer::FontRenderer(std::string path) : shader("shaders/text.vert", "shaders/text.frag") {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, image.width, image.height, 0, GL_RGBA, GL_UNSIGNED_BYTE, out.data()); // no idea if this will work
...
void FontRenderer::add_char(glm::vec2 pos_begin, glm::vec2 pos_end, glm::vec2 tex_pos_begin, glm::vec2 tex_pos_end, glm::vec4 color) {
    // x y
...
void FontRenderer::add_char(unsigned char last_c, unsigned char c, float &x, float &y, float size, glm::vec4 color) {
